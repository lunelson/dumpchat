import { DIAGNOSTICS_SCHEMA_NAME, DIAGNOSTICS_SCHEMA_VERSION, SITE_CONFIG } from "./config";
import {
  getVisibleUserNodes,
  isVisible,
  normalizeText,
...
export async function buildDiagnosticReport(site: Site): Promise<DiagnosticReport> {
  ...
  if (
    ...
    issues.push(
      "Copy buttons were found, but clipboard interception and fallback extraction both returned no content.",
    );
  }
...
export function deriveHealthStatus(input: {
  ...
  if (input.issues.length > 0 || input.usersExtracted === 0 || input.assistantsExtracted === 0) {
    return { level: "red", summary: "missing required content" };
  }

  const hasFallbackUse =
    input.assistantDebug.fromButtonFallbackCount > 0 || input.assistantDebug.usedFallbackCount > 0;
