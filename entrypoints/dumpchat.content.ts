...
import { buildDiagnosticReport, collectExportData } from "./lib/dumpchat/extraction";
...
async function runDiagnostics(button: HTMLButtonElement, site: Site): Promise<void> {
  try {
...
function showStatus(message: string): void {
  ...
  let status = currentStatus instanceof HTMLDivElement ? currentStatus : null;
  if (!status) {
...
function setHealthBadge(level: HealthLevel, detail: string): void {
  const currentBadge = document.getElementById(HEALTH_BADGE_ID);
  let badge = currentBadge instanceof HTMLDivElement ? currentBadge : null;
  if (!badge) {
...
async function buildMarkdown(site: Site): Promise<{ filename: string; markdown: string }> {
  ...
  for (let i = 0; i < turnCount; i += 1) {
    ...
    if (assistant) chunks.push(formatTurnSection(i + 1, "Assistant", assistant, location.href));
  }
